...
const Branches = (prop) => {
    ...
    const data = JSON.stringify({
        clientAddress: "127.0.0.0", acceptorCode: pass,
        address: "تهران خیابان جردن",
        city: "تهران ",
        code: "12",
        length: 10,
        offset: 5,
        search: "هر فیلدی که برای جستجو مد نظر هست",
        tel: "02166865498",
        name: "مرکزی"
    })
    ...
    const BankNationalcodeHandler = async () => {
        if (true) {
            ...
            const Auth = await fetch(`https://sbxapi.izbank.ir/private/general-services/v1/getBranches`, {
                method: 'POST',
                body: data,
                headers: {
    ...
    return (
        <Box sx={{ width: "75%", margin: "0 auto" }}>
            <Card sx={{ backgroundColor: "#FAFAFA" }}>
                ...
                <Box sx={{ display: "flex" }}>
                    <Typography sx={{ margin: "0 auto", border: "1px solid #ddd", borderRadius: "15px", fontSize: "1.1rem", p: 10, pt: 5, pb: 5 }}>لیست شعبه های بانک</Typography>
                </Box>
