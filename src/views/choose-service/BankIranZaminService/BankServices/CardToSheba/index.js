...
const CardToSheba = (prop) => {

    const [pass, setPass] = useState("9gshj-gfh-wej-lkjl")
    const [pan, setPan] = useState("5057851052918862")
    const data = JSON.stringify({ clientAddress: "127.0.0.0", acceptorCode: pass, pan: pan })

    const BankNationalcodeHandler = async () => {
        if (true) {
            // const client_Credentials = btoa("123:HB59BMAVFAZI0KFPXMOL")
            // console.log(client_Credentials)
            const Auth = await fetch(`https://sbxapi.izbank.ir/private/general-services/v1/depositToIban`, {
                method: 'POST',
                body: data,
                headers: {
    ...
    return (
        <Box sx={{ width: "75%", margin: "0 auto" }}>
            <Card sx={{ backgroundColor: "#FAFAFA" }}>
                ...
                <Box sx={{ display: "flex" }}>
                    <Typography sx={{ margin: "0 auto", border: "1px solid #ddd", borderRadius: "15px", fontSize: "1.1rem", p: 10, pt: 5, pb: 5 }}>تبدیل شماره کارت به شبا</Typography>
                </Box>
