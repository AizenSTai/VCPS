...
const DepositNumByNational = (prop) => {

    const [pass, setPass] = useState("9gshj-gfh-wej-lkjl")
    const [naitonalCode, setNationalCode] = useState("0000000000")
    const data = JSON.stringify({ clientAddress: "127.0.0.0", acceptorCode: pass, nationalCode: naitonalCode })

    const BankNationalcodeHandler = async () => {
        if (true) {
            // const client_Credentials = btoa("123:HB59BMAVFAZI0KFPXMOL")
            // console.log(client_Credentials)
            const Auth = await fetch(`https://sbxapi.izbank.ir/private/general-services/v1/inquiryDepositNumberByNId`, {
                method: 'POST',
                body: data,
                headers: {
    ...
    const labelIdentificationHandler = (prop) => {
        setIdentificationNumber(prop.target.value)
    }
    ...
    return (
        <Box sx={{ width: "75%", margin: "0 auto" }}>
            <Card sx={{ backgroundColor: "#FAFAFA" }}>
                ...
                <Box sx={{ display: "flex" }}>
                    <Typography sx={{ margin: "0 auto", border: "1px solid #ddd", borderRadius: "15px", fontSize: "1.1rem", p: 10, pt: 5, pb: 5 }}>استعلام موجودیت سپرده بر مبنای کد ملی</Typography>
                </Box>
                <Box sx={{ display: "flex", flexDirection: "column", justifyContent: "center", height: "auto", minHeight: "200px", textAlign: "center" }}>
                    ...
                    <Box sx={{ mb: 10 }}>
                        <TextField
                            ...
                            label="شماره شناسایی"
                            sx={{ direction: "ltr", width: "35%", minWidth: "250px" }}
                            onChange={labelIdentificationHandler}
                        />
                    </Box>
...
export default DepositNumByNational