...
const MatchNationalbySheba = (prop) => {

    const [pass, setPass] = useState("9gshj-gfh-wej-lkjl")
    const [sheba, setSheba] = useState("IR190090000657290000000000")
    const [naitonalCode, setNationalCode] = useState("0000000000")
    const data = JSON.stringify({ clientAddress: "127.0.0.0", acceptorCode: pass, iban: sheba, nationalCode: naitonalCode })

    const BankNationalcodeHandler = async () => {
        if (true) {
            // const client_Credentials = btoa("123:HB59BMAVFAZI0KFPXMOL")
            // console.log(client_Credentials)
            const Auth = await fetch(`https://sbxapi.izbank.ir/private/general-services/v1/otherBanksIbanInfo`, {
                method: 'POST',
                body: data,
                headers: {
    ...
    const labelShebaHandler = (prop) => {
        setSheba(prop.target.value)
    }

    const labelNationalCodeHandler = (prop) => {
        setNationalCode(prop.target.value)
    }
    ...
    return (
        <Box sx={{ width: "75%", margin: "0 auto" }}>
            <Card sx={{ backgroundColor: "#FAFAFA" }}>
                ...
                <Box sx={{ display: "flex" }}>
                    <Typography sx={{ margin: "0 auto", border: "1px solid #ddd", borderRadius: "15px", fontSize: "1.1rem", p: 10, pt: 5, pb: 5 }}>تطابق شماره ملی با شماره شبا</Typography>
                </Box>
                <Box sx={{ display: "flex", flexDirection: "column", justifyContent: "center", height: "auto", minHeight: "200px", textAlign: "center" }}>
                    ...
                    <Box sx={{ mb: 10 }}>
                        <TextField
                            ...
                            label="کد ملی"
                            sx={{ direction: "ltr", width: "35%", minWidth: "250px" }}
                            onChange={labelNationalCodeHandler}
                        // InputProps={{
                        //   startAdornment: <InputAdornment position="start"> +98 </InputAdornment>
                        // }}
                        />
...
export default MatchNationalbySheba