...
const BillInfo = (prop) => {
    ...
    const [payId, setPayId] = useState("9849729798")
    const [billId, setBillId] = useState("9849898229898")
    const [marchantId, setMarchantId] = useState("16849485")
    const data = JSON.stringify({
        clientAddress: "127.0.0.0", acceptorCode: pass, merchantId: marchantId, billId: billId, payId: payId
    })
    ...
    const BankNationalcodeHandler = async () => {
        if (true) {
            ...
            const Auth = await fetch(`https://sbxapi.izbank.ir/private/general-services/v1/getBillInfo`, {
                method: 'POST',
                body: data,
                headers: {
    ...
    const labelPayIdHandler = (prop) => {
        setPayId(prop.target.value)
    }
    const labelBillIdHandler = (prop) => {
        setBillId(prop.target.value)
    }
    const labelMarchantIdHandler = (prop) => {
        setMarchantId(prop.target.value)
    }
    ...
    return (
        <Box sx={{ width: "75%", margin: "0 auto" }}>
            <Card sx={{ backgroundColor: "#FAFAFA" }}>
                ...
                <Box sx={{ display: "flex" }}>
                    <Typography sx={{ margin: "0 auto", border: "1px solid #ddd", borderRadius: "15px", fontSize: "1.1rem", p: 10, pt: 5, pb: 5 }}>دریافت اطلاعات قبض</Typography>
                </Box>
                <Box sx={{ display: "flex", flexDirection: "column", justifyContent: "center", height: "auto", minHeight: "200px", textAlign: "center" }}>
                    ...
                    <Box sx={{ mb: 10 }}>
                        <TextField
                            ...
                            label="شناسه پذیرنده"
                            sx={{ direction: "ltr", width: "35%", minWidth: "250px" }}
                            onChange={labelMarchantIdHandler}
                        // InputProps={{
                        //   startAdornment: <InputAdornment position="start"> +98 </InputAdornment>
                        // }}
                        />
