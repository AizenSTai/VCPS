...
const DestinationDeposit = (prop) => {

    const [pass, setPass] = useState("9gshj-gfh-wej-lkjl")
    const [authorizedUserInfo, setAuthorizedUserInfo] = useState("eylad545q64qweq78asda54dasdada")
    const [mobile, setMobile] = useState("0000000000")
    const data = JSON.stringify({ clientAddress: "127.0.0.0", acceptorCode: pass, authorizedUserInfo: authorizedUserInfo, mobileNumber: mobile })

    const BankNationalcodeHandler = async () => {
        if (true) {
            // const client_Credentials = btoa("123:HB59BMAVFAZI0KFPXMOL")
            // console.log(client_Credentials)
            const Auth = await fetch(`https://sbxapi.izbank.ir/private/general-services/v1/getDestinationDeposits`, {
                method: 'POST',
                body: data,
                headers: {
    ...
    const labelAuthorizedUserInfoHandler = (prop) => {
        setAuthorizedUserInfo(prop.target.value)
    }

    const labelMobileHandler = (prop) => {
        setMobile(prop.target.value)
    }
    ...
    return (
        <Box sx={{ width: "75%", margin: "0 auto" }}>
            <Card sx={{ backgroundColor: "#FAFAFA" }}>
                ...
                <Box sx={{ display: "flex" }}>
                    <Typography sx={{ margin: "0 auto", border: "1px solid #ddd", borderRadius: "15px", fontSize: "1.1rem", p: 10, pt: 5, pb: 5 }}>لیست شماره سپرده های پر مخاطب</Typography>
                </Box>
                <Box sx={{ display: "flex", flexDirection: "column", justifyContent: "center", height: "auto", minHeight: "200px", textAlign: "center" }}>
                    <Box sx={{ mb: 10, mt: 10 }}>
                        <TextField
                            ...
                            onChange={labelAuthorizedUserInfoHandler}
                        // InputProps={{
...
export default DestinationDeposit