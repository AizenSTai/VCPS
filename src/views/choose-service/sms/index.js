...
import ThirdPage from 'src/views/choose-service/sms/ThirdPage'
...
const Sms = (prop) => {
  ...
  const ClickEv = (event) => {
    // localStorage.setItem("smslist", event)
    prop.onSetvalue('sms', event)
  ...
  return (
    <Box style={{ width: "100%", height: "100%", display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: 100 + 'px', zIndex: "1" }}>
      <Box>
        <Card sx={{ zIndex: 1 }}>
          <CardContent sx={{ padding: theme => `${theme.spacing(12, 9, 7)} !important` }}>
            <Box sx={{ mb: 6 }}>
              <Button variant='contained' sx={{ m: "10px", mt: "0px" }} onClick={prop.onBack}>بازگشت</Button>
            ...
            <Box>
              <Card >
                <Grid sm={12}>
                  <ThirdPage onBack={prop.onBack} onClick={ClickEv} />
                </Grid>
              </Card>
            </Box>
          </CardContent>
