...
const ServiceLink = props => {
  // ** Props
  const { title, onClick, countType, ShowOnClick, subtitle, color, icon, stats, trend, trendNumber, src, width, height } = props
  let num = 1
    ...
    <Card sx={{
      ...
      <Box sx={{ display: "flex", flexDirection: "column", width: "100%", height: "100%" }}>
        ...
        <Box sx={{ mt: 1 }}>
          ...
          {ShowOnClick && <Button fullWidth size='large' href={src} onClick={() => { countType != null && ((localStorage.getItem(countType) == null) ? localStorage.setItem(countType, 1) : localStorage.removeItem(countType), localStorage.setItem(countType, parseFloat(num) + 1)) }} target={'_blank'} variant='contained' sx={{ borderRadius: "15px", width: "auto", m: 5, mb: 2, mt: "0", fontSize: "0.7rem" }} >
            {subtitle}
          </Button>}
        </Box>
      </Box>
